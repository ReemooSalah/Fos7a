...
function buildPrompt(subjectId, age) {
  ...
  const map = {
    arabic:  `أنت مساعد تعليمي للأطفال السودانيين. اصنع 4 أسئلة تعليمية باللغة العربية مناسبة لطفل عمره من ${ageLabel}. الأسئلة عن الحروف والكلمات والقراءة البسيطة.`,
    english: `You are an educational assistant for Sudanese children. Create 4 simple English questions suitable for a child aged ${ageLabel}. Focus on letters, simple words, and basic vocabulary.`,
    math:    `أنت مساعد تعليمي. اصنع 4 أسئلة رياضيات بسيطة مناسبة لطفل عمره من ${ageLabel}. الأسئلة عن الأعداد والجمع والطرح البسيط.`,
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      <Card style={{ textAlign: "center", animation: shake ? "shake 0.5s" : "none", border: `2px solid ${subject.color}33` }}>
        {feedback && <div style={{ fontSize: 22, fontWeight: 900, marginBottom: 6, color: feedback === "correct" ? C.green : C.red, animation: "popIn 0.3s ease" }}>
